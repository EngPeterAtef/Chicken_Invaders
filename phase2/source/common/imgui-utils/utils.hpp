...
inline EnumMap comparison_functions = {{GL_ALWAYS, "GL_ALWAYS"},     {GL_NEVER, "GL_NEVER"},  {GL_EQUAL, "GL_EQUAL"},
                                       {GL_NOTEQUAL, "GL_NOTEQUAL"}, {GL_LESS, "GL_LESS"},    {GL_LEQUAL, "GL_LEQUAL"},
                                       {GL_GREATER, "GL_GREATER"},   {GL_GEQUAL, "GL_GEQUAL"}};

inline EnumMap face_windings = {{GL_CCW, "GL_CCW"}, {GL_CW, "GL_CW"}};

inline EnumMap facets = {{GL_FRONT, "GL_FRONT"}, {GL_BACK, "GL_BACK"}, {GL_FRONT_AND_BACK, "GL_FRONT_AND_BACK"}};
...
template <typename It> static void IteratorCombo(const char *label, std::string &selected, It begin, It end)
{
...
template <typename It> static void PairIteratorCombo(const char *label, std::string &selected, It begin, It end)
{
    if (ImGui::BeginCombo(label, selected.c_str()))
    {
        for (It it = begin; it != end; ++it)
...
static void ReorderableList(It begin, It end,
                            ...
                            std::function<void(size_t)> item_add, std::function<void(size_t)> item_delete,
