...
our::Texture2D *our::texture_utils::empty(GLenum format, glm::ivec2 size)
{
    ...
    texture->bind();
    // that last argument is the data that we want to send to the gpu which nullptr means that we want to send an empty
    // texture glTexStorage2D(GL_TEXTURE_2D, 0, format, size.x, size.y, 0, format, GL_UNSIGNED_BYTE, nullptr);
    GLsizei levels = 1;
